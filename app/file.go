...
func (a *App) GetFileInfosByChannelId(channelId string, page, perPage int) ([]*model.FileInfo, *model.AppError) {
	result, err := a.Srv().Store.FileInfo().GetByChannelId(channelId, perPage, page*perPage)
	if err != nil {
		return nil, model.NewAppError("GetFileByChannelId", "app.file_info.get_file_by_channel_id.app_error", nil, err.Error(), http.StatusBadRequest)
	}
	return result, nil
}
